...
@end
@implementation fasongGuideView
...
- (void)layoutSubviews {
    ...
    btnMaskRect.origin = CGPointMake(floor(SCREEN_W- 85), floor(SCREEN_H-40));
    ...
    _arrwoView.right = _btnMaskView.left-10;
...
{
    [self dismiss];
    [self.delegate fasongGuideViewNext];
...
- (UIImageView *)btnMaskView {
    if (!_btnMaskView) {
        ...
        imageView.image = [UIImage imageNamed:@"guideTuoyuan"];
...
- (UIImageView *)arrwoView {
    if (!_arrwoView) {
        UIImageView *imageView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"guide4s1"]];
        UIImage *image1 = [UIImage imageNamed:@"guide4s1"];
        UIImage *image2 = [UIImage imageNamed:@"guide4s2"];
