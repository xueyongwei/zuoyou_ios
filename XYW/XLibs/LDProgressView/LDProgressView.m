...
- (void)setAnimate:(NSNumber *)animate {
    ...
    if ([animate boolValue]) {
        self.timer = [NSTimer scheduledTimerWithTimeInterval:0.03 target:self selector:@selector(incrementOffset) userInfo:nil repeats:YES];
    } else if (self.timer) {
...
- (void)drawProgressBackground:(CGContextRef)context inRect:(CGRect)rect {
    ...
    if ([self.showBackgroundInnerShadow boolValue]) {
        ...
        CGContextSetShadowWithColor(context,
                CGSizeMake(xOffset + copysign(0.1, xOffset), yOffset + copysign(0.1, yOffset)), 5, [[UIColor clearColor] colorWithAlphaComponent:0.7].CGColor);
    }
