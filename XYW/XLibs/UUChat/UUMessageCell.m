...
//        headImageBackView.backgroundColor = [[UIColor grayColor] colorWithAlphaComponent:0.4];
        ...
        // 重试按钮
        self.btnRetryImage = [UIButton buttonWithType:UIButtonTypeCustom];
        [self.btnRetryImage setImage:[UIImage imageNamed:@"personal私信失败"] forState:UIControlStateNormal];
        [self.btnRetryImage addTarget:self action:@selector(btnRetryClick:) forControlEvents:UIControlEventTouchUpInside];
        [self.contentView addSubview:self.btnRetryImage];
...
-(void)btnRetryClick:(UIButton *)sender{
    if ([self.delegate respondsToSelector:@selector(retryClick:)])  {
        [self.delegate retryClick:self];
    }
}
...
//        UIMenuItem *delete=[[UIMenuItem alloc]initWithTitle:@"删除" action:@selector(clickToDeleteCell:)];
////        UIMenuItem *copy =[[UIMenuItem alloc]initWithTitle:@"复制" action:@selector(clickCopy:)];
//        menu.menuItems = @[delete];
        CGRect targetRect = self.btnContent.frame;
...
-(void)copy:(id)sender
{
    ...
    pboard.string = self.btnContent.titleLabel.text;
}
...
- (void)setMessageFrame:(UUMessageFrame *)messageFrame{
    ...
    // 1、设置时间
    self.labelTime.text = message.zyFormatTime;
...
//    self.btnHeadImage.frame = CGRectMake(2, 2, ChatIconWH-4, ChatIconWH-4);
    ...
    DbLog(@"message.strIcon:%@",message.strIcon);
    
    [self.btnHeadImage sd_setImageWithURL:[NSURL URLWithString:message.strIcon] forState:UIControlStateNormal placeholderImage:[UIImage imageNamed:@"1"]];
...
//                                 placeholderImage:[UIImage imageNamed:@"1"]];
    ...
    CGRect retryFrame = self.btnContent.frame;
    CGPoint center = self.btnContent.center;
    center.x = retryFrame.origin.x-=15;
    self.btnRetryImage.frame = CGRectMake(self.btnContent.frame.origin.x-40, self.btnContent.frame.size.height/2, 40, 30);
    self.btnRetryImage.center = center;
    self.btnRetryImage.hidden = !messageFrame.sendFaild;
    //背景气泡图
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        UIImage *normal;
        if (message.from == UUMessageFromMe) {
            normal = [UIImage imageNamed:@"chatto_bg_to"];
//            normal = [normal resizableImageWithCapInsets:UIEdgeInsetsMake(30, 25, 8, 18)];
            normal = [normal resizableImageWithCapInsets:UIEdgeInsetsMake(30, 25, 8, 18) resizingMode:UIImageResizingModeStretch];
        }
        else{
            normal = [UIImage imageNamed:@"chatto_bg_from"];
//            normal = [normal resizableImageWithCapInsets:UIEdgeInsetsMake(30, 18, 8, 25)];
            normal = [normal resizableImageWithCapInsets:UIEdgeInsetsMake(30, 25, 8, 18) resizingMode:UIImageResizingModeStretch];
        }
