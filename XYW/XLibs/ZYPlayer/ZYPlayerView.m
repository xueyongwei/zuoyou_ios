...
{
    self.playerLayerGravity = ZYPlayerLayerGravityResizeAspectFill;
}
...
/**
 ...
 如果设置了自动播放，那么就不会触发这个方法（在不需要流量监测的地方，直接使用自动播放）
 */
...
- (void)createTimer
{
    ...
    self.timeObserve = [self.player addPeriodicTimeObserverForInterval:CMTimeMake(1, 100) queue:nil usingBlock:^(CMTime time){
        ...
        if (loadedRanges.count > 0 && currentItem.duration.timescale != 0) {
            ...
            // 更新slider
            weakSelf.controlView.videoSlider.value     = CMTimeGetSeconds([currentItem currentTime]);
            weakSelf.controlView.videoSlider.maximumValue     =  totalTime;
...
- (void)animateShow
{
    if (self.isMaskShowing  || self.playDidEnd) {//已经显示了，或者播放完毕
        return;
    }
    if (self.state == ZFPlayerStatePlaying) {
...
- (void)bufferingSomeSecond
{
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1.0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        ...
        if (!self.playerItem.isPlaybackLikelyToKeepUp) {
            if (self.playDidEnd || self.isPauseByUser) {//播放完毕或者用户暂停
                return;
            }
            [self bufferingSomeSecond];
        }
...
- (void)play
{
    ...
    [_player play];
    [self autoFadeOutControlBar];
    [[NSNotificationCenter defaultCenter]postNotificationName:kZYPlayNotification object:nil];
}
...
- (void)pause
{
    ...
    self.isPauseByUser = YES;
    [_player pause];
    [[NSNotificationCenter defaultCenter]postNotificationName:kZYPauseNotification object:nil];
}
...
//        NSInteger dragedSeconds = floorf(total * slider.value);
        NSInteger dragedSeconds = slider.value;
 ...
 */
- (void)seekToTime:(CGFloat)dragedSeconds
{
...
//    if (self.isLocalVideo) {
        ...
        CMTime dragedCMTime = CMTimeMakeWithSeconds(dragedSeconds, self.playerItem.duration.timescale);
    DbLog(@"dragedCMTime %f",CMTimeGetSeconds(dragedCMTime));
        [self.player seekToTime:dragedCMTime toleranceBefore:CMTimeMake(1, self.playerItem.duration.timescale) toleranceAfter:CMTimeMake(1, self.playerItem.duration.timescale) completionHandler:^(BOOL finished) {
    ...
    if (placeholderImageUrl) {
        ...
        if (setBgImg) {
            [[SDWebImageDownloader sharedDownloader]downloadImageWithURL:[NSURL URLWithString:placeholderImageUrl] options:SDWebImageDownloaderLowPriority progress:nil completed:^(UIImage *image, NSData *data, NSError *error, BOOL finished) {
                ...
                wkSelf.layer.contentsGravity = @"resizeAspectFill";
                wkSelf.layer.masksToBounds = YES;
            }];
        }
