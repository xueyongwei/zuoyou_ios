...
- (void)refreshCell:(MKJChatModel *)model
{
    ...
    if (!model.isRight)
    {
        ...
        self.backView.frame = CGRectMake(60, 10, rec.size.width + 20, rec.size.height + 40);
        image = [UIImage imageNamed:@"yours"];
        headImage = [UIImage imageNamed:@"head.JPG"];
    }
    else // 模拟右边
    {
        self.headImageView.frame = CGRectMake(375 - 60, rec.size.height - 18, 50, 50);
        self.backView.frame = CGRectMake(375 - 60 - rec.size.width - 20, 10, rec.size.width + 20, rec.size.height + 40);
...
//        image.leftCapWidth		
    ...
    image = [image resizableImageWithCapInsets:UIEdgeInsetsMake(50, 30, 20, 30) resizingMode:UIImageResizingModeStretch];
//    image = [image stretchableImageWithLeftCapWidth:image.size.width*0.25 topCapHeight:image.size.height*0.75];
    self.backView.image = image;
//    self.backView.contentMode = UIViewContentModeScaleAspectFit;
    self.headImageView.image = headImage;
    // 文本内容的frame
    self.contentLabel.frame = CGRectMake(model.isRight ? 12 : 13, 5, rec.size.width, rec.size.height);
